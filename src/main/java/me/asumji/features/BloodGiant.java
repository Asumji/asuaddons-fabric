...
public class BloodGiant {
    ...
    public static void loadEntity(Giant giant) {
        if (!ConfigManager.getConfig().dungeonCategory.f7Accordion.diamanteAlert || !watcherSpawning) return;
        if (!giant.getItemBySlot(EquipmentSlot.CHEST).toString().equals("1 minecraft:diamond_chestplate")) return;
        watcherSpawning = false;
        Shortcuts.displayTitle("§bDiamante detected", "", 20);
        Shortcuts.queueClientMessage(Component.literal(AsuAddons.MOD_PREFIX + "§bDiamante Giant detected."));
    }
}